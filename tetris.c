...
int detectorColisao(char entrada, int linha, int coluna, peca peca, int matriz[22][12]){
    for(int k = 0; k < 4; k++){
        ...
        if(novaLinha == 21 || (entrada == 'd' && novaColuna == 10) || (entrada == 'a' && novaColuna == 1)) return 0;
        if(matriz[novaLinha-1][novaColuna] == 2 || (entrada == 'd' && novaColuna == 10) || (entrada == 'a' && novaColuna == 1)) return 2;
    }
...
int main()
{
    int pontuacao = 0, linha = 0, coluna = 0, novaLinha, novaColuna;
    ...
    for(int valorAleatorio = Aleatorio(pontuacao); TRUE; valorAleatorio = Aleatorio(pontuacao), linha = 0, coluna = 0){
        imprimirTela(matriz, pontuacao);


        for(; (detectorColisao(entrada, linha, 11, peca[valorAleatorio], matriz) == 1);){
            entrada = getch();
